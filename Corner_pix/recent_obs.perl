...
if($week_ago_date < 1){
	...
	if($week_ago_month < 1){
		$week_ago_month = 12;
		$week_ago_date = 31 - $week_ago_date;
		$pchk = 1;
	}else{
		if($week_ago_month == 2){
			$chk = 4.0 * int(0.25 * $uyear);
			if($chk == $uyear){
				$week_ago_date = 29 - $week_ago_date;
			}else{
				$week_ago_date = 28 - $week_ago_date;
			}
		}elsif($week_ago_month == 1 || $week_ago_month == 3 || $week_ago_month == 5 ||  $week_ago_month == 7
			|| $week_ago_month == 8 || $week_ago_month == 10){
				$week_ago_date = 31 - $week_ago_date;
		}else{
			$week_ago_date = 30 - $week_ago_date;
		}
	}
}
